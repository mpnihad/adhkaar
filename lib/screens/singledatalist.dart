...
import 'package:adhkaar/database/modelhelper/SingleDataHelper.dart';
...
class SingleDataList extends StatefulWidget {
  SingleDataList({
    Key key,
    this.animation,
    this.section,
  ...
  @override
  _SingleDataListState createState() => _SingleDataListState();
}

class _SingleDataListState extends State<SingleDataList>
  ...
  Future<List<SingleData>>singleDatalist ;
  int singleDataId;
  SingleDataHelper helper;
  SingleDataHelper duaHelper;
  List<ListCardModel> listcardmodel = [
    ListCardModel(
        "പ്രാർത്ഥനകളും മറ്റും അല്ലാഹു സ്വീകരിക്കുവാൻ അടിസ്ഥാനമായി വിശ്വസിക്കേണ്ട തൗഹീദ്",
        "https://halva1.000webhostapp.com/adkarimage/db23e1bb437a4f62c73c0e9a94bfc393.jpg"),
    ListCardModel("പ്രാർത്ഥനയുടെ ഗുണങ്ങൾ",
        "https://halva1.000webhostapp.com/adkarimage/image%20(2).png"),
    ListCardModel("പ്രാർത്ഥനയുടെ മര്യാദകൾ",
        "https://halva1.000webhostapp.com/adkarimage/1_-01-01-01_4x.png"),
    ListCardModel("പപ്രാർത്ഥനകൾക്ക് ഉത്തരം ലഭിക്കുന്ന സമയം",
        "https://halva1.000webhostapp.com/adkarimage/shot-cropped-1579324368600.png"),
    ListCardModel("ഉണരുമ്പോൾ",
        "https://halva1.000webhostapp.com/adkarimage/db23e1bb437a4f62c73c0e9a94bfc393.jpg")
  ];
...
//    }
  ...
  void initState() {
    var dbHelper = Helper();
    helper = SingleDataHelper(dbHelper.db);
    duaHelper = SingleDataHelper(dbHelper.db);
...
//                                builder: (context) => SettingsScreen()));
                            ...
                            body: Container(
                                ...
                                child: FutureBuilder<List<SingleData>>(
                                    future: getSingleData(helper,widget.section.id,Color(widget.section.color)),
                                    builder: (context, snapshot) {
                                      if (snapshot.hasData) {
...
//          ),
  ...
  Widget listItem(int index, Color pallete, List<SingleData> data) {
    SingleData subSingleData=data[index];
    animationController.forward();
    return AnimatedBuilder(
            ...
            child: new Transform(
              ...
              child: GestureDetector(
                onTap: () {
                  ...
                  DuaHeading duaHeading=new DuaHeading.forSingleData(subSingleData.relatedId,subSingleData.name,subSingleData.pallet);
...
//                              prevcolor: widget.prevColor)));
                ...
                child: createExpandedColumn(context, index, pallete,subSingleData),
              ),
            ),
          );
        });
  ...
  Future<bool> palletitem() async {
    ...
    List<SingleData> subSections=await getSectionSub(helper);
    int i = 0;
    for (SingleData subSection in subSections) {


      PaletteGenerator paletteGenerator =
      await PaletteGenerator.fromImageProvider(
        NetworkImage("https://res.cloudinary.com/halva/image/upload/v1588908206/adkhar/20190504_ryuaxg.jpg"),
        size: Size(50, 50),
      );
    ...
    setState(() {
      animation_colorstatus = true;
      if (animation_liststatus) {
        _ColorAnimationController1.forward();
      }
    });
  ...
  createExpandedColumn(BuildContext context, int index, Color pallete, SingleData subSection) {
    return Container(
      padding: index == 0 ? EdgeInsets.only(top: 8) : EdgeInsets.only(top: 0),
      child: ListCard(
        image: "https://res.cloudinary.com/halva/image/upload/v1588908206/adkhar/20190504_ryuaxg.jpg",
  ...
  createCollapsedColumn(BuildContext context, int index, Color pallete, Section subSection) {
    return SizedBox(
...
//              ),
  ...
  Widget listdown(int index, List<SingleData> data) {



    SingleData singleDataHeading = data[index];
          ...
          child: Container(
            ...
            child: InkWell(
              ...
              onTap: () {
                ...
                DuaHeading duaHeading=new DuaHeading.forSingleData(singleDataHeading.id,singleDataHeading.name,singleDataHeading.pallet);
                Navigator.of(context).push(
                  PageRouteBuilder(
                    pageBuilder: (BuildContext context,
                        Animation<double> animation,
  ...
  Future<List<SingleData>> getSectionSub(SingleDataHelper dbHelper) async {
    List<SingleData> gSection = [];
    await dbHelper.getSingleDatas(widget.section.id).then((value) {
      int  i=0;
      for (SingleData singleData in value) {
//        _colorTween1.insert(
//            i,
//            ColorTween(begin: Colors.transparent, end: whitebg)
//                .animate(_ColorAnimationController1));
        isExpanded.add(false);
        gSection.add(singleData);
        i++;
      }
    });
    return gSection;
  ...
  Future<List<SingleData>> getSingleData(SingleDataHelper dbHelper, int id,Color pallete) async {
    List<SingleData> gSingleData = [];
    await dbHelper.getSingleDatas(id).then((value) {
      int  i=0;
      for (SingleData singleData in value) {


        singleData.pallet=pallete;
        gSingleData.add(singleData);
        i++;
      }
    });
    return gSingleData;
